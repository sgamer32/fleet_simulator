...
# ==============================================================================
st.set_page_config(
    page_title="Locater | Real-Time Fleet & Delivery Simulator (v2)",
    page_icon="🚚",
    layout="wide",
...
# ==============================================================================
# INITIAL STATE & DEFAULT FLEET ROUTES
# ==============================================================================
DEFAULT_DEPOT = [40.7128, -74.0060] # New York Central Depot

PREDEFINED_ROUTES = {
    "Vehicle-101 (Van - Midtown Express)": {
        "driver": "Alex Rivera",
        "type": "Delivery Van",
        "color": "#2563EB", # Blue
        "capacity_items": 80,
        ...
        "base_speed_kmh": 42.0,
        "waypoints": [
            [40.7128, -74.0060], # Central Depot
            [40.7200, -73.9950], # Soho Stop
            [40.7350, -73.9910], # Union Square
            [40.7520, -73.9770], # Grand Central
            [40.7640, -73.9730]  # Central Park South Stop
        ]
    },
    "Vehicle-102 (Truck - Downtown Bulk)": {
        "driver": "Sarah Chen",
        "type": "Heavy Truck",
        "color": "#D97706", # Amber
        "capacity_items": 150,
        ...
        "waypoints": [
            [40.7128, -74.0060], # Central Depot
            [40.7050, -74.0090], # Financial District
            [40.7080, -74.0170], # Battery Park
            [40.7180, -74.0140], # Tribeca Hub
            [40.7280, -74.0080]  # West Village Stop
        ]
    },
    "Vehicle-103 (EV Bike - Rapid Courier)": {
        "driver": "Marcus Vance",
        "type": "Cargo E-Bike",
        "color": "#16A34A", # Green
        "capacity_items": 25,
        ...
        "waypoints": [
            [40.7128, -74.0060], # Central Depot
            [40.7180, -73.9880], # Lower East Side
            [40.7250, -73.9810], # East Village
            [40.7310, -73.9750], # Stuyvesant Town
            [40.7410, -73.9780]  # Kips Bay Stop
        ]
    }
}

TRAFFIC_HAZARD_PRESETS = {
    "Midtown Water Main Break": {
        "center": [40.7300, -73.9900],
        "radius_km": 0.9,
        "description": "Road closure at Soho/Union Sq corridor."
    },
    "Downtown Tunnel Construction": {
        "center": [40.7070, -74.0120],
        "radius_km": 0.7,
        "description": "Heavy congestion near Battery Park Tunnel."
    }
}
...
if "fleet_data" not in st.session_state:
    ...
    for v_id, v_info in PREDEFINED_ROUTES.items():
        total_dist = 0.0
        segments = []
...
def check_and_reroute_vehicles():
    """Detects active hazards along upcoming vehicle paths and automatically reroutes them."""
    ...
    for v_id, v_state in st.session_state.fleet_data.items():
        ...
        # Check proximity of current vehicle position or upcoming waypoints to any active hazard
        for h_name, h_info in st.session_state.active_hazards.items():
            h_center = h_info["center"]
            h_radius = h_info["radius_km"]
            
            # Check if current position or destination waypoints are inside hazard zone
            dist_to_hazard = calculate_haversine_distance(curr_pos, h_center)
            
            # Also check if upcoming waypoints intersect the hazard zone
            intersects_upcoming = any(
                calculate_haversine_distance(wp, h_center) <= h_radius 
                for wp in v_state["waypoints"][1:]
            )
            
            if dist_to_hazard <= (h_radius + 0.4) or intersects_upcoming:
                # Trigger Automated Rerouting!
                # Generate detour waypoints bypassing the hazard center
                curr_lat, curr_lon = curr_pos
                dest_lat, dest_lon = v_state["waypoints"][-1]
                
                # Offset detour via East River / FDR or West Side depending on hazard lon
                detour_offset_lon = 0.025 if h_center[1] < -73.9800 else -0.025
                bypass_pt1 = [curr_lat + 0.012, curr_lon + detour_offset_lon]
                bypass_pt2 = [dest_lat - 0.008, dest_lon + detour_offset_lon]
                
                new_wps = [curr_pos, bypass_pt1, bypass_pt2, [dest_lat, dest_lon]]
                
                # Recalculate distances from current location onwards
                new_segments = [
                ...
                v_state["progress_pct"] = 0.0 # Restart progress along new detour segment from current spot
                v_state["rerouted"] = True
                v_state["status"] = "Rerouted (Traffic Avoided)"
                v_state["reroute_reason"] = f"Avoided {h_name}"
                v_state["color"] = "#9333EA" # Highlight color (Purple) for rerouted vehicle
...
# ==============================================================================
def step_simulation():
    """Advances vehicle positions along route segments based on elapsed speed."""
    st.session_state.sim_time += 1
    step_minutes = 1.0 * st.session_state.speed_multiplier
    
    # Run automated hazard check & reroute before moving
    ...
    for v_id, v_state in st.session_state.fleet_data.items():
        ...
        # Current distance along total route
...
# HEADER & CONTROL BAR
# ==============================================================================
st.markdown('<div class="main-header">🚚 Locater | Real-Time Fleet & Delivery Simulator</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-header">Live GPS Tracking, Automated Re-Routing, & Traffic Congestion Bypass</div>', unsafe_allow_html=True)
...
with ctrl_col2:
    if st.button("🔄 Auto-Run 5 Steps", use_container_width=True):
        for _ in range(5):
            step_simulation()
...
# ==============================================================================
st.divider()
st.subheader("⚡ Automated Re-Routing & Traffic Hazard Controls")

haz_col1, haz_col2, haz_col3 = st.columns([2, 2, 2])

with haz_col1:
    selected_preset = st.selectbox("Simulate Traffic Incident", list(TRAFFIC_HAZARD_PRESETS.keys()))
    if st.button("⚠️ Trigger Road Closure Hazard", use_container_width=True):
        preset_info = TRAFFIC_HAZARD_PRESETS[selected_preset]
        st.session_state.active_hazards[selected_preset] = preset_info
        st.toast(f"Incident triggered: {selected_preset}", icon="⚠️")
        check_and_reroute_vehicles()
...
with haz_col2:
    ...
    if st.button("🧹 Clear All Traffic Hazards", use_container_width=True):
        st.session_state.active_hazards = {}
        st.toast("Cleared active road hazards", icon="🧹")

with haz_col3:
    if st.session_state.reroute_events:
        st.info(f"**Latest Reroute Event:**\n\n{st.session_state.reroute_events[-1]['msg']}")
    else:
        st.caption("No re-routing events triggered yet. Click 'Trigger Road Closure Hazard' to see automated re-routing in action.")
...
with map_col:
    st.subheader("🗺️ Live GPS Fleet Map with Detour Overlays")
    
    # Initialize Folium Map centered at NYC Central Depot
    m = folium.Map(location=[40.7300, -73.9900], zoom_start=13, tiles="CartoDB positron")
    
    # Add Depot Marker
    folium.Marker(
        location=DEFAULT_DEPOT,
        popup="<b>Central Logistics Depot</b><br>Origin Point",
        tooltip="Central Depot",
        icon=folium.Icon(color="black", icon="building", prefix="fa")
    ).add_to(m)
    
    # Render Active Traffic Hazard Overlay Circles
    for h_name, h_info in st.session_state.active_hazards.items():
        folium.Circle(
            location=h_info["center"],
            radius=h_info["radius_km"] * 1000, # meters
            color="#DC2626", # Red
            fill=True,
            fill_color="#EF4444",
            fill_opacity=0.35,
            popup=f"<b>⚠️ {h_name}</b><br>{h_info['description']}",
            tooltip=f"Hazard Zone: {h_name}"
        ).add_to(m)
    
    # Render Fleet Routes & Vehicle Moving Markers
    for v_id, v_state in st.session_state.fleet_data.items():
        wps = v_state["waypoints"]
        curr_pos = v_state["current_coord"]
...
with tele_col:
    ...
    for v_id, v_state in st.session_state.fleet_data.items():
        with st.expander(f"🚚 {v_id.split(' ')[0]} ({v_state['type']})", expanded=True):
            if v_state["rerouted"]:
                st.markdown(f"⚡ **Auto-Rerouted:** `{v_state['reroute_reason']}`")
            st.markdown(f"**Driver:** `{v_state['driver']}`")
...
with tab_logs:
    st.subheader("⚡ Automated Re-Routing Event Ledger")
    if st.session_state.reroute_events:
...
with tab_dispatch:
    st.subheader("Dispatch a New Vehicle to the Active Fleet")
    with st.form("dispatch_form"):
        col_d1, col_d2, col_d3 = st.columns(3)
        with col_d1:
            new_driver = st.text_input("Driver Name", value="Jordan Taylor")
            new_vtype = st.selectbox("Vehicle Type", ["Delivery Van", "Electric Scooter", "Heavy Cargo Truck"])
        with col_d2:
        ...
        st.markdown("**Select Route Waypoints (New York Area):**")
        w1_lat = st.number_input("Start Lat", value=40.7128, format="%.4f")
        w1_lon = st.number_input("Start Lon", value=-74.0060, format="%.4f")
        w2_lat = st.number_input("Destination Lat", value=40.7829, format="%.4f")
        w2_lon = st.number_input("Destination Lon", value=-73.9654, format="%.4f")
        
        submitted = st.form_submit_button("🚀 Launch Vehicle")
        if submitted:
            ...
            st.success(f"Vehicle `{new_id}` successfully dispatched to active fleet!")
            st.rerun()
...
with tab_about:
    ...
    * **Automated Geofenced Detour Calculation**: Detects road closure hazard zones and dynamically calculates alternate waypoint vectors.
    * **Path Interpolation**: Uses linear interpolation (`lerp`) across GPS coordinates between active route waypoints.
    * **Haversine Distance Model**: Recomputes remaining distances and dynamic ETAs whenever automated re-routing occurs.
    """)
